...
void gtkStoreAppend(gchar *data){
    ...
    if(strcmp(app_name, APP) == 0){
        ...
        gtk_tree_model_get (model_AppUsage, &iter_AppUsage,
                      COL_NPACK, &pack,
                      -1);
        g_value_init (&npack, G_TYPE_INT);
        g_value_set_int (&npack, (NPACK + pack));

        gtk_list_store_set_value(store_AppUsage,&iter_AppUsage,1,&npack);


        gtk_tree_model_get (model_AppUsage, &iter_AppUsage,
                      COL_TOTALLEN, &totlen,
                      -1);
        g_value_init (&totallen, G_TYPE_INT);
        g_value_set_int (&totallen, (TOTALLEN + totlen));

        gtk_list_store_set_value(store_AppUsage,&iter_AppUsage,2,&totallen);


        gtk_tree_model_get (model_AppUsage, &iter_AppUsage,
                      COL_F_NPACK, &fpack,
                      -1);
        g_value_init (&f_npack, G_TYPE_INT);
        g_value_set_int (&f_npack, (F_NPACK + fpack));

        gtk_list_store_set_value(store_AppUsage,&iter_AppUsage,3,&f_npack);


        gtk_tree_model_get (model_AppUsage, &iter_AppUsage,
                      COL_F_TOTALLEN, &ftotlen,
                      -1);
        g_value_init (&f_totallen, G_TYPE_INT);
        g_value_set_int (&f_totallen, (F_TOTALLEN + ftotlen));

        gtk_list_store_set_value(store_AppUsage,&iter_AppUsage,4,&f_totallen);


        gtk_tree_model_get (model_AppUsage, &iter_AppUsage,
                      COL_B_NPACK, &bpack,
                      -1);
        g_value_init (&b_npack, G_TYPE_INT);
        g_value_set_int (&b_npack, (B_NPACK + bpack));

        gtk_list_store_set_value(store_AppUsage,&iter_AppUsage,5,&b_npack);


        gtk_tree_model_get (model_AppUsage, &iter_AppUsage,
                      COL_B_TOTALLEN, &btotlen,
                      -1);
        g_value_init (&b_totallen, G_TYPE_INT);
        g_value_set_int (&b_totallen, (B_TOTALLEN + btotlen));

        gtk_list_store_set_value(store_AppUsage,&iter_AppUsage,6,&b_totallen);
...
static GtkTreeModel * create_and_fill_model_app (void){
    ...
    store_AppUsage = gtk_list_store_new (NUM_COLS,
                                    G_TYPE_STRING,
                                    G_TYPE_INT,
                                    G_TYPE_INT,
                                    G_TYPE_INT,
